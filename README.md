...
# Testing routing with mocks

To mock the routes with certain tags:
Add lines below to a profile (examplem mock for way 26648756)
...
to quickly rebuild the profiles for testing use below script followed by profile names(this will override profiles in data folder)

```
./prepare_test_data.sh bus tram
```

`profilesTests.js` compares profiles and saves the output in geojson format to files (need to adjust the names to match tested profiles in current example I was comparing bus and bus-with-temporal-filter profiles), also the commented lines would print and save to files all the shapes that would differ from each other

to run:
needs shapes.txt file inside of test directory(gtfs data file)

```
node tests/profilesTest.js
```
